...
from db.models import User, Schedule, Course, OTP
from schemas import (
    UserCreate, Token, ScheduleInput, SubjectInput, AdditionalConstraints,
...
from genetic_algorithm.hill_climbing import HillClimbingScheduler
from genetic_algorithm.simulated_annealing import SimulatedAnnealingScheduler
from genetic_algorithm.minimax_scheduler import MinimaxScheduler
...
def _run_scheduler_algorithm(  # AI_ALGORITHM
    algorithm: str,
    ...
    """Dispatches to the selected optimization algorithm. Returns (schedule, cost, convergence|None)."""
    algo = (algorithm or "ga").strip().lower()
    if algo == "ga":  # AI_ALGORITHM
...
# =================
async def _schedule_compute(input: ScheduleInput, current_user: User) -> Dict[str, Any]:  # AI_REPORT
    try:
        ...
        # Chạy thuật toán tối ưu được chọn
        final_schedule, final_cost, convergence_raw = _run_scheduler_algorithm(  # AI_ALGORITHM
            input.algorithm,
            subject_names,
            time_slots_for_ga,
            input.constraints,
        ...
        if convergence_out:
            out["convergence"] = convergence_out  # AI_REPORT

        if input.persist_schedule:
...
async def handle_schedule(
    ...
    return await _schedule_compute(input, current_user)


@app.post("/api/schedule/compare")
async def compare_scheduling_algorithms(
    input: ScheduleInput,
    current_user: User = Depends(get_current_user),
):
    """
    ...
    base_input = input.model_copy(update={"persist_schedule": False})  # AI_REPORT

    for algo in ["ga", "hill_climbing", "sa", "minimax"]:  # AI_ALGORITHM
        run_input = base_input.model_copy(update={"algorithm": algo})
        t0 = time.perf_counter()
        res = await _schedule_compute(run_input, current_user)
    ...
    return {"results": results, "best_algorithm": best_algorithm}


# --- Bộ dữ liệu benchmark cố định (báo cáo học thuật) ---  # AI_REPORT
BENCHMARK_CASES = [  # AI_REPORT
    {"name": "Nhỏ (3 môn)", "n_subjects": 3, "n_conflicts": 0},
    {"name": "Vừa (5 môn)", "n_subjects": 5, "n_conflicts": 1},
    {"name": "Trung bình (7 môn)", "n_subjects": 7, "n_conflicts": 2},
    {"name": "Lớn (10 môn)", "n_subjects": 10, "n_conflicts": 3},
    {"name": "Rất lớn (12 môn)", "n_subjects": 12, "n_conflicts": 4},
]


def _build_synthetic_benchmark_subjects(n_subjects: int, n_conflict_pairs: int) -> List[SubjectInput]:  # AI_REPORT
    """Tạo danh sách môn giả lập; các cặp đầu tiên bị ép trùng lịch để tạo xung đột."""
    days_pool = ["T2", "T3", "T4", "T5", "T6", "T7", "CN"]
    time_windows = [
        ("07:30", "09:15"),
        ("09:30", "11:15"),
        ("13:00", "14:45"),
    ...
    date_ranges = [
        ("2024-09-02", "2024-10-20"),
        ("2024-10-21", "2024-11-30"),
        ("2024-09-09", "2024-12-15"),
        ("2024-08-26", "2024-11-10"),
    ]
    rows: List[Dict[str, Any]] = []
    for i in range(n_subjects):
        st, en = random.choice(time_windows)
        d0, d1 = random.choice(date_ranges)
        rows.append({
            "name": f"Môn BM-{i + 1}",
            "start_time": st,
            "end_time": en,
            ...
            "instructor": f"GV {random.randint(1, 200)}",
            "start_date": d0,
            "end_date": d1,
    ...
    pairs = min(n_conflict_pairs, n_subjects // 2)
    for k in range(pairs):
        i, j = 2 * k, 2 * k + 1
        rows[i]["start_time"] = rows[j]["start_time"] = "08:00"
        rows[i]["end_time"] = rows[j]["end_time"] = "10:00"
        rows[i]["day"] = rows[j]["day"] = "T3"
        rows[i]["start_date"] = rows[j]["start_date"] = "2024-09-02"
        rows[i]["end_date"] = rows[j]["end_date"] = "2024-12-20"
        rows[i]["priority"] = 7
        rows[j]["priority"] = 4
    return [SubjectInput(**r) for r in rows]


@app.post("/api/schedule/benchmark")  # AI_REPORT
async def schedule_benchmark(current_user: User = Depends(get_current_user)):
    """Chạy 4 thuật toán trên 5 bộ dữ liệu mẫu cố định (không lưu DB)."""
    random.seed(42)
    benchmark_results: List[Dict[str, Any]] = []

    for case in BENCHMARK_CASES:
        subjects = _build_synthetic_benchmark_subjects(case["n_subjects"], case["n_conflicts"])
        base = ScheduleInput(
            subjects=subjects,
        ...
        for algo in ["ga", "hill_climbing", "sa", "minimax"]:
            run_input = base.model_copy(update={"algorithm": algo})
            t0 = time.perf_counter()
            res = await _schedule_compute(run_input, current_user)
            elapsed_ms = int((time.perf_counter() - t0) * 1000)
            case_results[algo] = {
